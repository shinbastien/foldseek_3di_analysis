...
from utils.pdz_pipeline_utils import (
    run_cmd, read_fasta_sequence, cleanse_createdb_file,
    extract_sequence_from_file_content, write_fasta_from_string,
    parse_tmalign_ranges
)
...
def main():
    ...
    if getattr(args, 'model_dir', None):
        ...
        if not model_variant_dirs:
            # fallback to 8f even if missing (preserves previous behavior)
            model_variant_dirs.append(('8f', (Path('encoders_and_tools/training_3di_gpu_8f')).resolve()))
    ...
    for suffix, (f1, f2) in new3di_by_var.items():
        ...
        # Special-case: for 10f we MUST use s_10f.mat and use pre-created 3di FASTA files.
        matrix_abs = None
        if suffix == '10f':
            # Try to find s_10f.mat: first in ssw_root, then in model_tmp
            s10_candidates = [
                ssw_root / 's_10f.mat',
                Path(model_tmp) / 's_10f.mat' if model_tmp else None,
            ]
            s10 = None
            for cand in s10_candidates:
            ...
            if not Path(ssw_exec).exists():
                ...
                sys.exit(1)
            if not s10:
                logger.error(f'Required matrix s_10f.mat not found. Please place it in {ssw_root} or {model_tmp}.')
                sys.exit(1)
            ...
            # choose matrix file: prefer per-variant matrix (s_<suffix>.mat) in model_tmp,
            # then global s_<suffix>.mat in ssw_root, then fall back to s.mat/sub_score.mat
            try:
                candidates = []
                if model_tmp is not None:
                    candidates.append(Path(model_tmp) / f's_{suffix}.mat')
