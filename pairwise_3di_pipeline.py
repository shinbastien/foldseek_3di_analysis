...
Usage:
    python pairwise_3di_pipeline.py pdb1.pdb pdb2.pdb \
        --model-dir training_3di_gpu_8f/tmp \
...
Behavior summary (defaults):
 ...
 - ssw_test for new: training_3di_gpu_8f/tmp/ssw_test
 - ssw_test for orig: training_3di_gpu_10f/tmp/ssw_test (falls back to 8f)
 - model_dir default: training_3di_gpu_8f/tmp
...
def extract_sequence_from_file_content(content: str) -> str:
    ...
    This is used for pdzdb_ss which user said is 3di sequence (maybe with null param at end)
    """
...
def main():
    ...
    if args.model_dir:
        ...
        if not variant_model_dirs:
            # fallback to the historical default (8f tmp)
            variant_model_dirs.append(('8f', (Path('training_3di_gpu_8f') / 'tmp').resolve()))
    ...
    for suffix, model_tmp in variant_model_dirs:
        ...
        matrix_path = Path(model_tmp) / 'sub_score.mat'
        if not matrix_path.exists():
            matrix_path = Path(model_tmp) / 's.mat'
        if not matrix_path.exists():
            print(f'Error: scoring matrix not found for variant {suffix} in {model_tmp} (expected sub_score.mat or s.mat).')
            sys.exit(1)
